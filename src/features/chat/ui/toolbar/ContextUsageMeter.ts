...
export class ContextUsageMeter {
  ...
  setCompacting(compacting: boolean): void {
    ...
    this.container.toggleClass('claudian-context-meter--compacting', compacting);
    this.updateTooltip();
  }
  ...
  private render() {
    ...
    this.container.addClass('claudian-clickable');
  }
  ...
  private showPopup(): void {
    ...
    this.popupEl = document.body.createDiv({ cls: 'claudian-context-popup' });

    const headerEl = this.popupEl.createDiv({ cls: 'claudian-context-popup-header' });
    headerEl.createSpan({ text: 'Context Usage' });

    const closeBtn = headerEl.createSpan({ cls: 'claudian-context-popup-close' });
    closeBtn.setText('×');
    ...
    const contentEl = this.popupEl.createDiv({ cls: 'claudian-context-popup-content' });

    if (this.currentUsage) {
      const tokensEl = contentEl.createDiv({ cls: 'claudian-context-popup-row' });
      tokensEl.createSpan({ cls: 'claudian-context-popup-label', text: 'Context:' });
      tokensEl.createSpan({
        cls: 'claudian-context-popup-value',
        text: `${this.formatTokens(this.currentUsage.contextTokens)} / ${this.formatTokens(this.currentUsage.contextWindow)}`,
      });

      const percentEl = contentEl.createDiv({ cls: 'claudian-context-popup-row' });
      percentEl.createSpan({ cls: 'claudian-context-popup-label', text: 'Usage:' });
      ...
      if (this.currentUsage.inputTokens !== undefined) {
        const inputEl = contentEl.createDiv({ cls: 'claudian-context-popup-row' });
        inputEl.createSpan({ cls: 'claudian-context-popup-label', text: 'Input tokens:' });
        inputEl.createSpan({
          cls: 'claudian-context-popup-value',
    ...
    if (supportsCompact && this.callbacks?.onCompact) {
      const compactEl = contentEl.createDiv({ cls: 'claudian-context-popup-actions' });
      const compactBtn = compactEl.createEl('button', {
        cls: 'claudian-context-popup-compact-btn',
    ...
    const wrapperRect = this.container.closest('.claudian-input-wrapper')?.getBoundingClientRect() ?? null;
